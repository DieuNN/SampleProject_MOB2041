...
class Database(private val mContext: Context) :
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("CREATE TABLE $TABLE_CALL_CARD (ID INTEGER PRIMARY KEY AUTOINCREMENT, CUSTOMER_NAME TEXT, BOOK_NAME TEXT, GENRE TEXT, LIBRARIAN_NAME TEXT, BORROW_TIME TEXT, IS_RETURNED NOT NULL CHECK (IS_RETURNED IN(0, 1)), FOREIGN KEY (CUSTOMER_NAME) REFERENCES $TABLE_CUSTOMER ON DELETE CASCADE, FOREIGN KEY (BOOK_NAME) REFERENCES $TABLE_BOOK ON DELETE CASCADE, FOREIGN KEY (GENRE) REFERENCES $TABLE_GENRE ON DELETE CASCADE, FOREIGN KEY (LIBRARIAN_NAME) REFERENCES $TABLE_LIBRARIAN ON DELETE CASCADE)")
    }
